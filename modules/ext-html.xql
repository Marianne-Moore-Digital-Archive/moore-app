...
declare function pmf:create-table($table as element()) {
    let $id := $table/@xml:id
    return
        <pb-table-grid source="api/table/{$id}" resizable="resizable" limit="25" css-path="{$config:context-path}/resources/css/gridjs">
            {for $column in $table/tei:row[@role eq 'label']/tei:cell
            let $label := $column/string()
            return 
                <pb-table-column label="{$label}" property="{replace($label, '\s+', '_')}" sort=""></pb-table-column>
            }
        </pb-table-grid>
    };