...
module namespace model="http://www.tei-c.org/pm/models/moore/latex";
    ...
    $input !         (
            ...
            return
                    ...
                    case element(castItem) return
                        ...
                        latex:listItem($config, ., ("tei-castItem", "tei-castItem1", css:map-rend-to-class(.)), ., ())
                    case element(item) return
                        latex:listItem($config, ., ("tei-item", "tei-item1", css:map-rend-to-class(.)), ., ())
                    case element(teiHeader) return
                        if ($parameters?mode eq 'metadata-doc') then
                            latex:pass-through($config, ., ("tei-teiHeader", "tei-teiHeader1", css:map-rend-to-class(.)), (//fileDesc/titleStmt/editor[1], //origDate, //msDesc))
                        else
                            latex:metadata($config, ., ("tei-teiHeader", "tei-teiHeader2", css:map-rend-to-class(.)), .)
                    case element(figure) return
                        if (head or @rendition='simple:display') then
                            latex:block($config, ., ("tei-figure", "tei-figure1", css:map-rend-to-class(.)), .)
                        else
                            latex:inline($config, ., ("tei-figure", "tei-figure2", css:map-rend-to-class(.)), .)
                    case element(supplied) return
                        if (parent::choice) then
                            latex:inline($config, ., ("tei-supplied", "tei-supplied1", css:map-rend-to-class(.)), .)
                        else
                            if (@reason) then
                                let $params := 
                                    map {
                                ...
                                return
                                                                latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-supplied", "tei-supplied2", css:map-rend-to-class(.)), $content)
                            else
                                latex:inline($config, ., ("tei-supplied", "tei-supplied3", css:map-rend-to-class(.)), .)
                    case element(ptr) return
                        if (parent::notatedMusic) then
                            ...
                            return
                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-ptr", "tei-ptr1", css:map-rend-to-class(.)), $content)
                        else
                            if ((parent::place or parent::person) and @target) then
                                ...
                                return
                                                                latex:listItem(map:merge(($config, map:entry("template", true()))), ., ("tei-ptr", "tei-ptr2", css:map-rend-to-class(.)), $content, ())
                            else
                                if (@target) then
                                    latex:link($config, ., ("tei-ptr", "tei-ptr3", css:map-rend-to-class(.)), if (@type) then @type else ., @target, map {})
                                else
                                    $config?apply($config, ./node())
                    case element(signed) return
                        if (parent::closer) then
                            latex:block($config, ., ("tei-signed", "tei-signed1", "signed", css:map-rend-to-class(.)), .)
                        else
                            latex:inline($config, ., ("tei-signed", "tei-signed2", css:map-rend-to-class(.)), .)
                    case element(pb) return
                        if ($parameters?mode= ('toc', 'title')) then
                            latex:inline($config, ., ("tei-pb", "tei-pb1", css:map-rend-to-class(.)), ext-html:page-number(.))
                        else
                            if (@facs) then
                                ...
                                if (starts-with(@facs, 'iiif:')) then
                                    ...
                                    latex:break($config, ., css:get-rendition(., ("tei-pb", "tei-pb4", css:map-rend-to-class(.))), ., 'page', (concat(if(@n) then concat(@n,' ') else '',if(@facs) then                   concat('@',@facs) else '')))
                    case element(TEI) return
                        if ($parameters?mode='breadcrumb') then
                            latex:pass-through($config, ., ("tei-TEI", "tei-TEI1", css:map-rend-to-class(.)), teiHeader)
                        else
                            if ($parameters?mode='edition-navigation' and descendant::correspContext) then
                                ...
                                latex:pass-through($config, ., ("tei-TEI", "tei-TEI2", css:map-rend-to-class(.)), descendant::correspContext)
                            else
                                if ($parameters?mode = 'register') then
                                    ...
                                    return
                                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-TEI", "tei-TEI3", css:map-rend-to-class(.)), $content)
                                else
                                    latex:document($config, ., ("tei-TEI", "tei-TEI4", css:map-rend-to-class(.)), .)
                    case element(anchor) return
                        if ($parameters?mode=('toc', 'breadcrumb')) then
                            latex:omit($config, ., ("tei-anchor", "tei-anchor1", css:map-rend-to-class(.)), .)
                        else
                            if (@corresp) then
                                (: Process notes with corresp attribute (assuming the note is stand off, somewhere in the same document) :)
                                latex:note($config, ., ("tei-anchor", "tei-anchor2", css:map-rend-to-class(.)), let $target := @corresp return root($parameters?root)/id($target)/node(), (), @n)
                            else
                                if (@type='note') then
                                    (: Process notes with @type=note (assuming the note is stand off, in the listAnnotation section in the same document) :)
                                    latex:note($config, ., ("tei-anchor", "tei-anchor3", css:map-rend-to-class(.)), let $target := '#' || @xml:id return root($parameters?root)//listAnnotation/note[@target=$target]/node(), (), let $target := '#' || @xml:id return root($parameters?root)//listAnnotation/note[@target=$target]/@n/string())
                                else
                                    if (@xml:id) then
                                        latex:anchor($config, ., ("tei-anchor", "tei-anchor4", css:map-rend-to-class(.)), ., @xml:id)
                                    else
                                        $config?apply($config, ./node())
                    case element(formula) return
                        if (@rendition='simple:display') then
                            latex:block($config, ., ("tei-formula", "tei-formula1", css:map-rend-to-class(.)), .)
                        else
                            if (@rend="display") then
                                ...
                                return
                                                                latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-formula", "tei-formula2", css:map-rend-to-class(.)), $content)
                            else
                                if (@rend='display') then
                                    (: No function found for behavior: webcomponent :)
                                    $config?apply($config, ./node())
                                    ...
                                    return
                                                                        latex:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-formula", "tei-formula3", css:map-rend-to-class(.)), $content)
                    case element(choice) return
                        if (sic and corr) then
                            latex:alternate($config, ., ("tei-choice", "tei-choice1", css:map-rend-to-class(.)), ., corr[1], sic[1])
                        else
                            if (abbr and expan) then
                                latex:alternate($config, ., ("tei-choice", "tei-choice2", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                            else
                                if (orig and reg) then
                                    latex:alternate($config, ., ("tei-choice", "tei-choice3", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                else
                                    $config?apply($config, ./node())
                    case element(hi) return
                        ...
                        return
                                                latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-hi", "tei-hi1", css:map-rend-to-class(.)), $content)
                    case element(note) return
                        if (@target) then
                            latex:note($config, ., ("tei-note", "tei-note1", css:map-rend-to-class(.)), ., (), 'g')
                        else
                            if (not($parameters?mode eq 'metadata-doc')) then
                                latex:note($config, ., ("tei-note", "tei-note2", css:map-rend-to-class(.)), ., @place, 'n')
                            else
                                $config?apply($config, ./node())
                    case element(code) return
                        latex:inline($config, ., ("tei-code", "tei-code1", css:map-rend-to-class(.)), .)
                    case element(death) return
                        latex:inline($config, ., ("tei-death", "tei-death1", css:map-rend-to-class(.)), .)
                    case element(dateline) return
                        latex:block($config, ., ("tei-dateline", "tei-dateline1", css:map-rend-to-class(.)), .)
                    case element(back) return
                        latex:block($config, ., ("tei-back", "tei-back1", css:map-rend-to-class(.)), .)
                    case element(correspContext) return
                        if ($parameters?static) then
                            ...
                            return
                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-correspContext", "tei-correspContext1", css:map-rend-to-class(.)), $content)
                        else
                            ...
                            return
                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-correspContext", "tei-correspContext2", css:map-rend-to-class(.)), $content)
                    case element(del) return
                        latex:inline($config, ., ("tei-del", "tei-del1", "nowrap", css:map-rend-to-class(.)), for $x in ./node() return if (contains(path($x), 'text()')) then ext-html:replaces-spaces($x) else if (contains(path($x), 'comment()')) then () else $x)
                    case element(trailer) return
                        latex:block($config, ., ("tei-trailer", "tei-trailer1", css:map-rend-to-class(.)), .)
                    case element(titlePart) return
                        latex:block($config, ., css:get-rendition(., ("tei-titlePart", "tei-titlePart1", css:map-rend-to-class(.))), .)
                    case element(ab) return
                        latex:paragraph($config, ., ("tei-ab", "tei-ab1", css:map-rend-to-class(.)), .)
                    case element(app) return
                        if ($parameters?mode='toc') then
                            latex:inline($config, ., ("tei-app", "tei-app1", css:map-rend-to-class(.)), lem)
                        else
                            latex:alternate($config, ., ("tei-app", "tei-app2", css:map-rend-to-class(.)), ., lem, rdg)
                    case element(revisionDesc) return
                        if ($parameters?display='browse') then
                            latex:omit($config, ., ("tei-revisionDesc", "tei-revisionDesc1", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(birth) return
                        latex:inline($config, ., ("tei-birth", "tei-birth1", css:map-rend-to-class(.)), .)
                    case element(subst) return
                        (: explicitly set content to avoid whitespace issues :)
                        latex:inline($config, ., ("tei-subst", "tei-subst1", css:map-rend-to-class(.)), (del, add))
                    case element(roleDesc) return
                        latex:block($config, ., ("tei-roleDesc", "tei-roleDesc1", css:map-rend-to-class(.)), .)
                    case element(opener) return
                        latex:block($config, ., ("tei-opener", "tei-opener1", css:map-rend-to-class(.)), .)
                    case element(speaker) return
                        latex:block($config, ., ("tei-speaker", "tei-speaker1", css:map-rend-to-class(.)), .)
                    case element(region) return
                        latex:inline($config, ., ("tei-region", "tei-region1", css:map-rend-to-class(.)), .)
                    case element(imprimatur) return
                        latex:block($config, ., ("tei-imprimatur", "tei-imprimatur1", css:map-rend-to-class(.)), .)
                    case element(foreign) return
                        latex:inline($config, ., ("tei-foreign", "tei-foreign1", "nowrap", css:map-rend-to-class(.)), for $x in ./node() return if (contains(path($x), 'text()')) then ext-html:replaces-spaces($x) else if (contains(path($x), 'comment()')) then () else $x)
                    case element(mei:mdiv) return
                        ...
                        return
                                                latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-mei_mdiv", "tei-mei_mdiv1", css:map-rend-to-class(.)), $content)
                    case element(fileDesc) return
                        if ($parameters?display='browse') then
                            (
                                latex:block($config, ., ("tei-fileDesc", "tei-fileDesc1", css:map-rend-to-class(.)), (titleStmt, editionStmt, publicationStmt)),
                                (: Output abstract containing demo description :)
                                latex:block($config, ., ("tei-fileDesc", "tei-fileDesc2", "sample-description", css:map-rend-to-class(.)), ../profileDesc/abstract)
                            )

                        else
                            latex:block($config, ., ("tei-fileDesc", "tei-fileDesc3", css:map-rend-to-class(.)), .)
                    case element(seg) return
                        latex:inline($config, ., css:get-rendition(., ("tei-seg", "tei-seg1", css:map-rend-to-class(.))), .)
                    case element(notatedMusic) return
                        latex:figure($config, ., ("tei-notatedMusic", "tei-notatedMusic1", css:map-rend-to-class(.)), (ptr, mei:mdiv), label)
                    case element(profileDesc) return
                        if ($parameters?display='browse') then
                            latex:omit($config, ., ("tei-profileDesc", "tei-profileDesc1", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(place) return
                        if ($parameters?mode='register-overview') then
                            ...
                            return
                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-place", "tei-place1", css:map-rend-to-class(.)), $content)
                        else
                            if ($parameters?mode='register') then
                                ...
                                return
                                                                latex:listItem(map:merge(($config, map:entry("template", true()))), ., ("tei-place", "tei-place2", css:map-rend-to-class(.)), $content, ())
                            else
                                (
                                    let $params := 
                                        map {
                                    ...
                                    return
                                                                        latex:heading(map:merge(($config, map:entry("template", true()))), ., ("tei-place", "tei-place3", css:map-rend-to-class(.)), $content, ()),
                                    (: Additional notes :)
                                    latex:paragraph($config, ., ("tei-place", "tei-place4", css:map-rend-to-class(.)), note),
                                    (: Links to wikidata, wikipedia etc. :)
                                    latex:list($config, ., ("tei-place", "tei-place5", "links", css:map-rend-to-class(.)), ptr, ()),
                                    if ($parameters?mode='register-details') then
                                        ...
                                        return
                                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-place", "tei-place6", css:map-rend-to-class(.)), $content)
                                    else
                                        ()
                                )

                    case element(text) return
                        latex:body($config, ., ("tei-text", "tei-text1", css:map-rend-to-class(.)), .)
                    case element(floatingText) return
                        latex:block($config, ., ("tei-floatingText", "tei-floatingText1", css:map-rend-to-class(.)), .)
                    case element(sp) return
                        latex:block($config, ., ("tei-sp", "tei-sp1", css:map-rend-to-class(.)), .)
                    case element(table) return
                        latex:table($config, ., ("tei-table", "tei-table1", css:map-rend-to-class(.)), ., map {})
                    case element(group) return
                        latex:block($config, ., ("tei-group", "tei-group1", css:map-rend-to-class(.)), .)
                    case element(cb) return
                        latex:break($config, ., ("tei-cb", "tei-cb1", css:map-rend-to-class(.)), ., 'column', @n)
                    case element(licence) return
                        if (@target) then
                            latex:link($config, ., ("tei-licence", "tei-licence1", "licence", css:map-rend-to-class(.)), 'Licence', @target, map {})
                        else
                            $config?apply($config, ./node())
                    case element(orgName) return
                        if (@key and $parameters?mode='register-details') then
                            latex:inline($config, ., ("tei-orgName", "tei-orgName3", css:map-rend-to-class(.)), let $key:= head((@key, @ref)) return id($key, collection($global:register-root)/id('pb-organizations'))/placeName[@type='main']/string())
                        else
                            if ($parameters?display='browse' or $parameters?mode='breadcrumb' or $parameters?mode='toc') then
                                latex:inline($config, ., ("tei-orgName", "tei-orgName4", css:map-rend-to-class(.)), .)
                            else
                                if ($parameters?mode='register') then
                                    (: Register sidebar :)
                                    let $params := 
                                    ...
                                    return
                                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-orgName", "tei-orgName5", css:map-rend-to-class(.)), $content)
                                else
                                    if (@type='variant') then
                                        latex:inline($config, ., ("tei-orgName", "tei-orgName6", "name", "variant", css:map-rend-to-class(.)), .)
                                    else
                                        if (parent::org) then
                                            latex:inline($config, ., ("tei-orgName", "tei-orgName7", css:map-rend-to-class(.)), .)
                                        else
                                            if (ancestor::correspContext) then
                                                latex:inline($config, ., ("tei-orgName", "tei-orgName8", css:map-rend-to-class(.)), .)
                                            else
                                                if (@key or @ref) then
                                                    latex:alternate($config, ., ("tei-orgName", "tei-orgName9", "orgName", "context", css:map-rend-to-class(.)), ., ., let $key:= head((@key, @ref)) return id($key, collection($global:register-root)/id('pb-organizations')))
                                                else
                                                    latex:inline($config, ., ("tei-orgName", "tei-orgName10", css:map-rend-to-class(.)), .)
                    case element(listBibl) return
                        if (bibl) then
                            latex:list($config, ., ("tei-listBibl", "tei-listBibl1", css:map-rend-to-class(.)), ., ())
                        else
                            latex:block($config, ., ("tei-listBibl", "tei-listBibl2", css:map-rend-to-class(.)), .)
                    case element(g) return
                        if (not(text())) then
                            latex:glyph($config, ., ("tei-g", "tei-g1", css:map-rend-to-class(.)), .)
                        else
                            latex:inline($config, ., ("tei-g", "tei-g2", css:map-rend-to-class(.)), .)
                    case element(author) return
                        if (ancestor::teiHeader) then
                            latex:block($config, ., ("tei-author", "tei-author1", css:map-rend-to-class(.)), .)
                        else
                            latex:inline($config, ., ("tei-author", "tei-author2", css:map-rend-to-class(.)), .)
                    case element(castList) return
                        if (child::*) then
                            latex:list($config, ., css:get-rendition(., ("tei-castList", "tei-castList1", css:map-rend-to-class(.))), castItem, ())
                        else
                            $config?apply($config, ./node())
                    case element(l) return
                        if (not(node())) then
                            (: Create line break for empty lines :)
                            latex:break($config, ., ("tei-l", "tei-l1", css:map-rend-to-class(.)), ., 'line', ())
                        else
                            if (@rend) then
                                let $params := 
                                    map {
                                ...
                                return
                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-l", "tei-l2", "line", css:map-rend-to-class(.))), $content)
                            else
                                let $params := 
                                    map {
                                        "content": for $x in ./node() return if (contains(path($x), 'text()')) then ext-html:replaces-spaces($x) else if (contains(path($x), 'comment()')) then () else $x
                                ...
                                return
                                                                latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-l", "tei-l3", "line", css:map-rend-to-class(.)), $content)
                    case element(closer) return
                        latex:block($config, ., ("tei-closer", "tei-closer1", css:map-rend-to-class(.)), .)
                    case element(p) return
                        if (parent::msDesc and not(preceding-sibling::p)) then
                            ...
                            return
                                                        latex:paragraph(map:merge(($config, map:entry("template", true()))), ., ("tei-p", "tei-p1", css:map-rend-to-class(.)), $content)
                        else
                            latex:paragraph($config, ., css:get-rendition(., ("tei-p", "tei-p3", css:map-rend-to-class(.))), .)
                    case element(list) return
                        latex:list($config, ., css:get-rendition(., ("tei-list", "tei-list1", css:map-rend-to-class(.))), item, ())
                    case element(q) return
                        if (l) then
                            latex:block($config, ., css:get-rendition(., ("tei-q", "tei-q1", "block-quote", css:map-rend-to-class(.))), .)
                        else
                            if (ancestor::p or ancestor::cell or ancestor::cit) then
                                (: If inside a paragraph, table cell or citation then rendered inline :)
                                latex:inline($config, ., css:get-rendition(., ("tei-q", "tei-q2", "inline-quote", css:map-rend-to-class(.))), .)
                            else
                                latex:block($config, ., css:get-rendition(., ("tei-q", "tei-q3", "block-quote", css:map-rend-to-class(.))), .)
                    case element(epigraph) return
                        latex:block($config, ., ("tei-epigraph", "tei-epigraph1", css:map-rend-to-class(.)), .)
                    case element(lb) return
                        latex:break($config, ., css:get-rendition(., ("tei-lb", "tei-lb1", css:map-rend-to-class(.))), ., 'line', @n)
                    case element(docTitle) return
                        latex:block($config, ., css:get-rendition(., ("tei-docTitle", "tei-docTitle1", css:map-rend-to-class(.))), .)
                    case element(w) return
                        latex:inline($config, ., ("tei-w", "tei-w1", css:map-rend-to-class(.)), .)
                    case element(person) return
                        if (parent::listPerson and $parameters?mode='register-overview') then
                            ...
                            return
                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-person", "tei-person1", css:map-rend-to-class(.)), $content)
                        else
                            if (parent::listPerson and $parameters?mode='register') then
                                (: Register sidebar :)
                                latex:listItem($config, ., ("tei-person", "tei-person2", css:map-rend-to-class(.)), persName[@type=('main', 'full')], ())
                            else
                                if (parent::listPerson) then
                                    (
                                        ...
                                        return
                                                                                latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-person", "tei-person3", css:map-rend-to-class(.)), $content),
                                        (: Variant names :)
                                        latex:block($config, ., ("tei-person", "tei-person4", css:map-rend-to-class(.)), persName[@type='variant']),
                                        (: Images :)
                                        latex:block($config, ., ("tei-person", "tei-person5", css:map-rend-to-class(.)), figure),
                                        (: Additional notes :)
                                        latex:paragraph($config, ., ("tei-person", "tei-person6", css:map-rend-to-class(.)), note),
                                        (: Links to wikidata, wikipedia etc. :)
                                        latex:list($config, ., ("tei-person", "tei-person7", css:map-rend-to-class(.)), ptr, ()),
                                        if ($parameters?mode='register-details') then
                                            (: List of letters (in register details only); trying to match by key or ref attributes :)
                                            let $params := 
                                                map {
                                            ...
                                            return
                                                                                        latex:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person", "tei-person8", css:map-rend-to-class(.)), $content)
                                        else
                                            ()
                                    )
                    ...
                    case element(titlePage) return
                        latex:block($config, ., css:get-rendition(., ("tei-titlePage", "tei-titlePage1", css:map-rend-to-class(.))), .)
                    case element(stage) return
                        latex:block($config, ., ("tei-stage", "tei-stage1", css:map-rend-to-class(.)), .)
                    case element(persName) return
                        if (@key and $parameters?mode='register-details') then
                            latex:inline($config, ., ("tei-persName", "tei-persName3", css:map-rend-to-class(.)), let $key:= head((@key, @ref)) return id($key, collection($global:register-root)/id('pb-persons'))/persName[@type='main']/string())
                        else
                            if ($parameters?display='browse' or $parameters?mode='breadcrumb' or $parameters?mode='toc') then
                                latex:inline($config, ., ("tei-persName", "tei-persName4", css:map-rend-to-class(.)), .)
                            else
                                if ($parameters?mode='register') then
                                    (: Register sidebar :)
                                    let $params := 
                                    ...
                                    return
                                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-persName", "tei-persName5", css:map-rend-to-class(.)), $content)
                                else
                                    if (@type='variant') then
                                        latex:inline($config, ., ("tei-persName", "tei-persName6", "name", "variant", css:map-rend-to-class(.)), .)
                                    else
                                        if (parent::person) then
                                            latex:inline($config, ., ("tei-persName", "tei-persName7", css:map-rend-to-class(.)), .)
                                        else
                                            if (ancestor::correspContext) then
                                                (: do not resolve correspContext descriptions :)
                                                latex:inline($config, ., ("tei-persName", "tei-persName8", css:map-rend-to-class(.)), .)
                                            else
                                                if (@key or @ref) then
                                                    latex:alternate($config, ., ("tei-persName", "tei-persName9", "persName", "context", css:map-rend-to-class(.)), ., ., let $key:= head((@key, @ref))                          let $resolved := id($key, collection($global:register-root)/id('pb-persons')) return if (count($resolved)) then $resolved else $key)
                                                else
                                                    latex:inline($config, ., ("tei-persName", "tei-persName10", css:map-rend-to-class(.)), .)
                    case element(lg) return
                        if (@type eq 'sideways') then
                            latex:block($config, ., ("tei-lg", "tei-lg1", css:map-rend-to-class(.)), .)
                        else
                            if (@type eq 'vertical') then
                                latex:block($config, ., ("tei-lg", "tei-lg2", css:map-rend-to-class(.)), .)
                            else
                                latex:block($config, ., ("tei-lg", "tei-lg3", css:map-rend-to-class(.)), .)
                    case element(front) return
                        latex:block($config, ., ("tei-front", "tei-front1", css:map-rend-to-class(.)), .)
                    case element(publicationStmt) return
                        if ($parameters?display='browse') then
                            latex:block($config, ., ("tei-publicationStmt", "tei-publicationStmt1", css:map-rend-to-class(.)), .//availability)
                        else
                            latex:block($config, ., ("tei-publicationStmt", "tei-publicationStmt2", css:map-rend-to-class(.)), .)
                    case element(placeName) return
                        if (@key and $parameters?mode='register-details') then
                            latex:inline($config, ., ("tei-placeName", "tei-placeName3", css:map-rend-to-class(.)), let $key:= head((@key, @ref)) return id($key, collection($global:register-root)/id('pb-places'))/placeName[@type='main']/string())
                        else
                            if ($parameters?display='browse' or $parameters?mode='breadcrumb' or $parameters?mode='toc') then
                                latex:inline($config, ., ("tei-placeName", "tei-placeName4", css:map-rend-to-class(.)), .)
                            else
                                if ($parameters?mode='register') then
                                    (: Register sidebar :)
                                    let $params := 
                                    ...
                                    return
                                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-placeName", "tei-placeName5", css:map-rend-to-class(.)), $content)
                                else
                                    if (@type='variant') then
                                        latex:inline($config, ., ("tei-placeName", "tei-placeName6", "name", "variant", css:map-rend-to-class(.)), .)
                                    else
                                        if (parent::place) then
                                            latex:inline($config, ., ("tei-placeName", "tei-placeName7", css:map-rend-to-class(.)), .)
                                        else
                                            if (ancestor::correspContext) then
                                                latex:inline($config, ., ("tei-placeName", "tei-placeName8", css:map-rend-to-class(.)), .)
                                            else
                                                if (@key or @ref) then
                                                    latex:alternate($config, ., ("tei-placeName", "tei-placeName9", "placeName", "context", css:map-rend-to-class(.)), ., ., let $key:= head((@key, @ref))                          let $resolved := id($key, collection($global:register-root)/id('pb-places')) return if (count($resolved)) then $resolved else $key)
                                                else
                                                    latex:inline($config, ., ("tei-placeName", "tei-placeName10", css:map-rend-to-class(.)), .)
                    case element(role) return
                        latex:block($config, ., ("tei-role", "tei-role1", css:map-rend-to-class(.)), .)
                    case element(damage) return
                        latex:inline($config, ., ("tei-damage", "tei-damage1", css:map-rend-to-class(.)), .)
                    case element(country) return
                        latex:inline($config, ., ("tei-country", "tei-country1", css:map-rend-to-class(.)), .)
                    case element(postscript) return
                        latex:block($config, ., ("tei-postscript", "tei-postscript1", css:map-rend-to-class(.)), .)
                    case element(edition) return
                        if (ancestor::teiHeader) then
                            latex:block($config, ., ("tei-edition", "tei-edition1", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(cell) return
                        (: Insert table cell. :)
                        latex:cell($config, ., ("tei-cell", "tei-cell1", css:map-rend-to-class(.)), ., ())
                    case element(div) return
                        if ($parameters?mode='title') then
                            latex:pass-through($config, ., ("tei-div", "tei-div1", css:map-rend-to-class(.)), descendant::pb)
                        else
                            if (lg[@type = ('vertical', 'sideways')]) then
                                latex:block($config, ., ("tei-div", "tei-div2", css:map-rend-to-class(.)), .)
                            else
                                latex:block($config, ., ("tei-div", "tei-div3", css:map-rend-to-class(.)), .)
                    case element(graphic) return
                        latex:graphic($config, ., ("tei-graphic", "tei-graphic1", css:map-rend-to-class(.)), ., @url, @width, @height, @scale, desc)
                    case element(ref) return
                        if (@subtype='placeholder') then
                            ...
                            return
                                                        latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-ref", "tei-ref1", css:map-rend-to-class(.)), $content)
                        else
                            if (parent::correspContext and @type) then
                                ...
                                return
                                                                latex:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-ref", "tei-ref2", css:map-rend-to-class(.)), $content)
                            else
                                if (@target) then
                                    latex:link($config, ., ("tei-ref", "tei-ref3", css:map-rend-to-class(.)), ., @target, map {})
                                else
                                    latex:inline($config, ., ("tei-ref", "tei-ref4", css:map-rend-to-class(.)), .)
                    case element(add) return
                        latex:inline($config, ., ("tei-add", "tei-add1", css:map-rend-to-class(.)), .)
                    case element(head) return
                        if ($parameters?display='browse' or $parameters?mode='toc') then
                            ...
                            latex:inline($config, ., ("tei-head", "tei-head1", css:map-rend-to-class(.)), replace(string-join(.//text()[not(parent::ref)]), '^(.+?)[^\w]*$', '$1'))
                        else
                            if (parent::figure) then
                                latex:block($config, ., ("tei-head", "tei-head2", "figure-head", css:map-rend-to-class(.)), .)
                            else
                                if (parent::table) then
                                    latex:block($config, ., ("tei-head", "tei-head3", "table-head", css:map-rend-to-class(.)), .)
                                else
                                    if (parent::lg) then
                                        latex:block($config, ., ("tei-head", "tei-head4", "lg-head", css:map-rend-to-class(.)), .)
                                    else
                                        if (parent::list) then
                                            latex:block($config, ., ("tei-head", "tei-head5", "list-head", css:map-rend-to-class(.)), .)
                                        else
                                            if (parent::div) then
                                                latex:heading($config, ., ("tei-head", "tei-head6", css:map-rend-to-class(.)), ., count(ancestor::div))
                                            else
                                                latex:block($config, ., ("tei-head", "tei-head7", css:map-rend-to-class(.)), .)
                    case element(ex) return
                        if (ancestor::expan) then
                            latex:inline($config, ., ("tei-ex", "tei-ex1", css:map-rend-to-class(.)), ('(', ., ')'))
                        else
                            $config?apply($config, ./node())
                    case element(time) return
                        latex:inline($config, ., ("tei-time", "tei-time1", css:map-rend-to-class(.)), .)
                    case element(castGroup) return
                        if (child::*) then
                            (: Treat as a list :)
                            latex:list($config, ., ("tei-castGroup", "tei-castGroup1", css:map-rend-to-class(.)), castItem|castGroup, ())
                        else
                            $config?apply($config, ./node())
                    case element(bibl) return
                        if (parent::listBibl) then
                            latex:listItem($config, ., ("tei-bibl", "tei-bibl1", css:map-rend-to-class(.)), ., ())
                        else
                            if (@type = ('start', 'end')) then
                                latex:omit($config, ., ("tei-bibl", "tei-bibl2", css:map-rend-to-class(.)), .)
                            else
                                latex:inline($config, ., ("tei-bibl", "tei-bibl3", css:map-rend-to-class(.)), .)
                    case element(unclear) return
                        latex:inline($config, ., ("tei-unclear", "tei-unclear1", "unclear", "nowrap", css:map-rend-to-class(.)), for $x in ./node() return if (contains(path($x), 'text()')) then ext-html:replaces-spaces($x) else if (contains(path($x), 'comment()')) then () else $x)
                    case element(salute) return
                        if (parent::closer) then
                            latex:inline($config, ., ("tei-salute", "tei-salute1", css:map-rend-to-class(.)), .)
                        else
                            latex:block($config, ., ("tei-salute", "tei-salute2", css:map-rend-to-class(.)), .)
                    case element(title) return
                        if (@type) then
                            latex:omit($config, ., ("tei-title", "tei-title1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?display='browse') then
                                latex:heading($config, ., ("tei-title", "tei-title2", css:map-rend-to-class(.)), ., 5)
                            else
                                if (parent::titleStmt/parent::fileDesc) then
                                    (
                                        if (preceding-sibling::title) then
                                            latex:text($config, ., ("tei-title", "tei-title3", css:map-rend-to-class(.)), ' — ')
                                        else
                                            (),
                                        latex:inline($config, ., ("tei-title", "tei-title4", css:map-rend-to-class(.)), .)
                                    )

                                else
                                    if (not(@level) and parent::bibl) then
                                        latex:inline($config, ., ("tei-title", "tei-title5", css:map-rend-to-class(.)), .)
                                    else
                                        if (@level='m' or not(@level)) then
                                            (
                                                latex:inline($config, ., ("tei-title", "tei-title6", css:map-rend-to-class(.)), .),
                                                if (ancestor::biblFull) then
                                                    latex:text($config, ., ("tei-title", "tei-title7", css:map-rend-to-class(.)), ', ')
                                                else
                                                    ()
                                            )
                                            ...
                                            if (@level='s' or @level='j') then
                                                (
                                                    latex:inline($config, ., ("tei-title", "tei-title8", css:map-rend-to-class(.)), .),
                                                    if (following-sibling::* and     (  ancestor::biblFull)) then
                                                        latex:text($config, ., ("tei-title", "tei-title9", css:map-rend-to-class(.)), ', ')
                                                    else
                                                        ()
                                                )
                                                ...
                                                if (@level='u' or @level='a') then
                                                    (
                                                        latex:inline($config, ., ("tei-title", "tei-title10", css:map-rend-to-class(.)), .),
                                                        if (following-sibling::* and     (    ancestor::biblFull)) then
                                                            latex:text($config, ., ("tei-title", "tei-title11", css:map-rend-to-class(.)), '. ')
                                                        else
                                                            ()
                                                    )

                                                else
                                                    latex:inline($config, ., ("tei-title", "tei-title12", css:map-rend-to-class(.)), .)
                    case element(date) return
                        if (($parameters?mode eq 'metadata-doc') and (ancestor::publicationStmt)) then
                            ...
                            return
                                                        latex:paragraph(map:merge(($config, map:entry("template", true()))), ., ("tei-date", "tei-date1", css:map-rend-to-class(.)), $content)
                        else
                            if ($parameters?mode eq 'metadata-doc') then
                                latex:paragraph($config, ., ("tei-date", "tei-date2", css:map-rend-to-class(.)), .)
                            else
                                latex:inline($config, ., ("tei-date", "tei-date4", css:map-rend-to-class(.)), .)
                    case element(argument) return
                        latex:block($config, ., ("tei-argument", "tei-argument1", css:map-rend-to-class(.)), .)
                    case element(corr) return
                        if (parent::choice and count(parent::*/*) gt 1) then
                            (: simple inline, if in parent choice. :)
                            latex:inline($config, ., ("tei-corr", "tei-corr1", css:map-rend-to-class(.)), .)
                        else
                            latex:inline($config, ., ("tei-corr", "tei-corr2", css:map-rend-to-class(.)), .)
                    case element(cit) return
                        if (child::quote and child::bibl) then
                            (: Insert citation :)
                            latex:cit($config, ., ("tei-cit", "tei-cit1", css:map-rend-to-class(.)), ., ())
                        else
                            $config?apply($config, ./node())
                    case element(titleStmt) return
                        if ($parameters?mode eq 'breadcrumb') then
                            latex:inline($config, ., ("tei-titleStmt", "tei-titleStmt1", css:map-rend-to-class(.)), head(title)/string())
                        else
                            (: No function found for behavior: meta :)
                            $config?apply($config, ./node())
                    case element(sic) return
                        if (parent::choice) then
                            latex:inline($config, ., ("tei-sic", "tei-sic1", css:map-rend-to-class(.)), .)
                        else
                            latex:inline($config, ., ("tei-sic", "tei-sic2", "sic", css:map-rend-to-class(.)), .)
                    case element(spGrp) return
                        latex:block($config, ., ("tei-spGrp", "tei-spGrp1", css:map-rend-to-class(.)), .)
                    case element(body) return
                        (
                            latex:index($config, ., ("tei-body", "tei-body1", css:map-rend-to-class(.)), ., 'toc'),
                            latex:block($config, ., ("tei-body", "tei-body2", css:map-rend-to-class(.)), .)
                        )

                    case element(fw) return
                        if (ancestor::p or ancestor::ab) then
                            latex:inline($config, ., ("tei-fw", "tei-fw1", css:map-rend-to-class(.)), .)
                        else
                            latex:block($config, ., ("tei-fw", "tei-fw2", css:map-rend-to-class(.)), .)
                    case element(encodingDesc) return
                        if ($parameters?display='browse') then
                            latex:omit($config, ., ("tei-encodingDesc", "tei-encodingDesc1", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(quote) return
                        if (@type = ('start', 'end')) then
                            latex:omit($config, ., css:get-rendition(., ("tei-quote", "tei-quote1", css:map-rend-to-class(.))), .)
                        else
                            latex:inline($config, ., ("tei-quote", "tei-quote2", "nowrap", css:map-rend-to-class(.)), for $x in ./node() return if (contains(path($x), 'text()')) then ext-html:replaces-spaces($x) else if (contains(path($x), 'comment()')) then () else $x)
                    case element(gap) return
                        if (desc) then
                            latex:inline($config, ., ("tei-gap", "tei-gap1", css:map-rend-to-class(.)), .)
                        else
                            if (@extent) then
                                latex:inline($config, ., ("tei-gap", "tei-gap2", css:map-rend-to-class(.)), for $x in 1 to xs:integer(number(@extent)) return '⍰')
                            else
                                latex:inline($config, ., ("tei-gap", "tei-gap3", css:map-rend-to-class(.)), .)
                    case element(correspAction) return
                        if (@type='sent') then
                            latex:inline($config, ., ("tei-correspAction", "tei-correspAction1", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    case element(row) return
                        if (@role='label') then
                            latex:row($config, ., ("tei-row", "tei-row1", "row-label", css:map-rend-to-class(.)), .)
                        else
                            latex:row($config, ., ("tei-row", "tei-row2", css:map-rend-to-class(.)), .)
                    case element(byline) return
                        latex:block($config, ., ("tei-byline", "tei-byline1", css:map-rend-to-class(.)), .)
                    case element(editor) return
                        if ($parameters?mode eq 'metadata-doc') then
                            ...
                            return
                                                        latex:listItem(map:merge(($config, map:entry("template", true()))), ., ("tei-editor", "tei-editor1", css:map-rend-to-class(.)), $content, ())
                        else
                            $config?apply($config, ./node())
                    case element(msDesc) return
                        if ($parameters?mode eq 'metadata-doc') then
                            ...
                            return
                                                        latex:listItem(map:merge(($config, map:entry("template", true()))), ., ("tei-msDesc", "tei-msDesc1", css:map-rend-to-class(.)), $content, ())
                        else
                            $config?apply($config, ./node())
                    case element(repository) return
                        if ($parameters?mode eq 'metadata-doc') then
                            ...
                            return
                                                        latex:paragraph(map:merge(($config, map:entry("template", true()))), ., ("tei-repository", "tei-repository1", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(idno) return
                        if ($parameters?mode eq 'metadata-doc') then
                            ...
                            return
                                                        latex:paragraph(map:merge(($config, map:entry("template", true()))), ., ("tei-idno", "tei-idno1", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(respStmt) return
                        if ($parameters?mode eq 'metadata-doc') then
                            ...
                            return
                                                        latex:paragraph(map:merge(($config, map:entry("template", true()))), ., ("tei-respStmt", "tei-respStmt1", css:map-rend-to-class(.)), $content)
                        else
                            $config?apply($config, ./node())
                    case element(origDate) return
                        if ($parameters?mode eq 'metadata-doc') then
                            ...
                            return
                                                        latex:listItem(map:merge(($config, map:entry("template", true()))), ., ("tei-origDate", "tei-origDate1", css:map-rend-to-class(.)), $content, ())
                        else
                            $config?apply($config, ./node())
                    case element() return
