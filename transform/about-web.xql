(:~
    ...
    ODD: /db/apps/moore/resources/odd/about.odd
 :)
xquery version "3.1";

module namespace model="http://www.tei-c.org/pm/models/about/web";
...
(: generated template function for element spec: figure :)
declare %private function model:template-figure1($config as map(*), $node as node()*, $params as map(*)) {
    <t xmlns=""><span class="tei-figure right" style="{$config?apply-children($config, $node, $params?style)}">{$config?apply-children($config, $node, $params?content)}</span></t>/*
};
(: generated template function for element spec: figure :)
declare %private function model:template-figure2($config as map(*), $node as node()*, $params as map(*)) {
    <t xmlns=""><span class="tei-figure left" style="{$config?apply-children($config, $node, $params?style)}">{$config?apply-children($config, $node, $params?content)}</span></t>/*
};
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?mode='breadcrumb') then
                            html:inline($config, ., ("tei-teiHeader", "tei-teiHeader3", css:map-rend-to-class(.)), fileDesc/titleStmt)                            => model:map($node, $trackIds)
                        else
                    ...
                    case element(figure) return
                        if (@rend eq 'right') then
                            let $params := 
                                map {
                                    "style": let $width := if (graphic/@width) then 'width:' || graphic/@width || '; ' else () let $height := if (graphic/@height) then 'height:' || graphic/@width || '; ' else () return 'float:right; margin: 0 0 1em 1em; ' || $width || $height,
                                    "content": .
                                }

                                                        let $content := 
                                model:template-figure1($config, ., $params)
                            return
                                                        html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-figure", "tei-figure1", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                            if (@rend eq 'left') then
                                let $params := 
                                    map {
                                        "style": let $width := if (graphic/@width) then 'width:' || graphic/@width || '; ' else () let $height := if (graphic/@height) then 'height:' || graphic/@width || '; ' else () return 'font-size: smaller;float:left; margin: 0 1em 1em 0; ' || $width || $height,
                                        "content": .
                                    }

                                                                let $content := 
                                    model:template-figure2($config, ., $params)
                                return
                                                                html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-figure", "tei-figure2", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                                html:block($config, ., ("tei-figure", "tei-figure3", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                    ...
                    case element(pb) return
                        if ($parameters?iiif-viewer='pb-facsimile') then
                            ...
                            html:webcomponent($config, ., ("tei-pb", "tei-pb2", css:map-rend-to-class(.)), 'Page ' || @n, 'pb-facs-link', map {"facs": replace(@facs, '^iiif:(.*)$', '$1'), "emit": 'transcription'})                            => model:map($node, $trackIds)
                        else
                            if (@facs) then
                    ...
                    case element(note) return
                        if (parent::person or parent::place) then
                            html:inline($config, ., ("tei-note", "tei-note1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if ($parameters?mode='toc' or ancestor::listAnnotation) then
                                html:omit($config, ., ("tei-note", "tei-note2", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                html:note($config, ., ("tei-note", "tei-note3", css:map-rend-to-class(.)), ., @place, @n)                                => model:map($node, $trackIds)
                    ...
                    case element(del) return
                        html:inline($config, ., ("tei-del", "tei-del1", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(foreign) return
                        html:inline($config, ., ("tei-foreign", "tei-foreign1", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(table) return
                        if (@xml:id) then
                            html:pass-through($config, ., ("tei-table", "tei-table1", css:map-rend-to-class(.)), ext-html:create-table(.))                            => model:map($node, $trackIds)
                        else
                            html:table($config, ., ("tei-table", "tei-table2", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                    ...
                    case element(l) return
                        html:block($config, ., css:get-rendition(., ("tei-l", "tei-l1", css:map-rend-to-class(.))), .)                        => model:map($node, $trackIds)
                    case element(closer) return
                        html:block($config, ., ("tei-closer", "tei-closer1", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    case element(p) return
                        html:paragraph($config, ., css:get-rendition(., ("tei-p", "tei-p2", css:map-rend-to-class(.))), .)                        => model:map($node, $trackIds)
                    ...
                    case element(cell) return
                        if ($parameters?mode eq 'table-data') then
                            html:inline($config, ., ("tei-cell", "tei-cell1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            (: Insert table cell. :)
                            html:cell($config, ., ("tei-cell", "tei-cell2", css:map-rend-to-class(.)), ., ())                            => model:map($node, $trackIds)
                    case element(div) return
                        if ($parameters?mode = 'register') then
                            ...
                            return
                                                        html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-div", "tei-div1", css:map-rend-to-class(.)), $content)                            => model:map($node, $trackIds)
                        else
                            if (parent::body or parent::front or parent::back) then
                                html:section($config, ., ("tei-div", "tei-div2", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                            else
                                html:block($config, ., ("tei-div", "tei-div3", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                    case element(graphic) return
                        if (parent::figure/@rend = ('right', 'left')) then
                            html:graphic($config, ., ("tei-graphic", "tei-graphic1", css:map-rend-to-class(.)), ., @url, (), (), (), head((desc, parent::figure/figDesc)))                            => model:map($node, $trackIds)
                        else
                            html:graphic($config, ., ("tei-graphic", "tei-graphic2", css:map-rend-to-class(.)), ., @url, @width, @height, @scale, head((desc, parent::figure/figDesc)))                            => model:map($node, $trackIds)
                    ...
                    case element(bibl) return
                        if ($parameters?mode='register-details') then
                            (
                                html:block($config, ., ("tei-bibl", "tei-bibl1", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds),
                                if ($parameters?mode='register-details') then
                                    (: List of letters (in register details only); trying to match by key or ref attributes :)
                                    let $params := 
                                        map {
                                    ...
                                    return
                                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-bibl", "tei-bibl2", css:map-rend-to-class(.)), $content)                                    => model:map($node, $trackIds)
                                else
                                    ()
                            )
                            ...
                            if (parent::listBibl and $parameters?mode='register-overview') then
                                ...
                                return
                                                                html:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-bibl", "tei-bibl3", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                                $config?apply($config, ./node())
                    case element(unclear) return
                        html:inline($config, ., ("tei-unclear", "tei-unclear1", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
                    ...
                    case element(title) return
                        if ($parameters?mode='breadcrumb') then
                            html:inline($config, ., ("tei-title", "tei-title1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if ($parameters?mode='register-details' and $parameters?static) then
                                html:link($config, ., ("tei-title", "tei-title2", css:map-rend-to-class(.)), string(), $parameters?context-path || "/letters/" || util:document-name(.) || '/1/index.html', (), map {})                                => model:map($node, $trackIds)
                            else
                                if ($parameters?mode='register-details') then
                                    (: create a link to the document, respecting address-by-id configuration :)
                                    html:link($config, ., ("tei-title", "tei-title3", css:map-rend-to-class(.)), string(),                          if ($global:address-by-id) then                             '../' || root(.)/descendant-or-self::TEI/@xml:id                                                      else                                 '../' || substring-after(document-uri(root(.)), $global:data-root || '/')                          , (), map {})                                    => model:map($node, $trackIds)
                                else
                                    if ($parameters?display='browse') then
                                        html:heading($config, ., ("tei-title", "tei-title4", css:map-rend-to-class(.)), ., 5)                                        => model:map($node, $trackIds)
                                    else
                                        if (@key or @ref) then
                                            html:alternate($config, ., ("tei-title", "tei-title5", css:map-rend-to-class(.)), ., ., let $key:= head((@key, @ref))  return collection($global:register-root)/id($key), map {})                                            => model:map($node, $trackIds)
                                        else
                    ...
                    case element(date) return
                        if ($parameters?display='browse' or $parameters?mode='breadcrumb') then
                            html:inline($config, ., ("tei-date", "tei-date1", css:map-rend-to-class(.)), .)                            => model:map($node, $trackIds)
                        else
                            if (@when) then
                                html:alternate($config, ., ("tei-date", "tei-date2", css:map-rend-to-class(.)), ., if (normalize-space(.)) then . else @when/string(), try { format-date(xs:date(@when), '[D1o] [MNn] [Y]', (session:get-attribute('lang'), 'en')[1], (), ()) } catch * { @when/string() }, map {})                                => model:map($node, $trackIds)
                            else
                                html:inline($config, ., ("tei-date", "tei-date3", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                    ...
                    case element(titleStmt) return
                        if ($parameters?mode='breadcrumb') then
                            ...
                            html:inline($config, ., ("tei-titleStmt", "tei-titleStmt3", css:map-rend-to-class(.)), title[1])                            => model:map($node, $trackIds)
                        else
                            if ($parameters?display='browse') then
                                (
                                    if ($parameters?static) then
                                        html:link($config, ., ("tei-titleStmt", "tei-titleStmt4", css:map-rend-to-class(.)), title[1], $parameters?context-path || '/' || util:document-name(.) || '/1/index.html', (), map {})                                        => model:map($node, $trackIds)
                                    else
                                        (),
                                    if (not($parameters?static)) then
                                        html:link($config, ., ("tei-titleStmt", "tei-titleStmt5", css:map-rend-to-class(.)), title[1], $parameters?doc, (), map {})                                        => model:map($node, $trackIds)
                                    else
                                        (),
                                    html:block($config, ., ("tei-titleStmt", "tei-titleStmt6", css:map-rend-to-class(.)), subsequence(title, 2))                                    => model:map($node, $trackIds),
                                    html:block($config, ., ("tei-titleStmt", "tei-titleStmt7", css:map-rend-to-class(.)), author)                                    => model:map($node, $trackIds)
                                )

                            else
                                if ($parameters?mode='register-details') then
                                    (: List of letters in register detail view :)
                                    html:listItem($config, ., ("tei-titleStmt", "tei-titleStmt8", css:map-rend-to-class(.)), head((title[@type='main'], title)), ())                                    => model:map($node, $trackIds)
                                else
                                    html:block($config, ., ("tei-titleStmt", "tei-titleStmt9", css:map-rend-to-class(.)), .)                                    => model:map($node, $trackIds)
                    ...
                    case element(quote) return
                        if (ancestor::p or ancestor::cell or ancestor::cit) then
                            (: If inside a paragraph, table cell or citation then rendered inline :)
                            html:inline($config, ., css:get-rendition(., ("tei-quote", "tei-quote1", "inline-quote", css:map-rend-to-class(.))), .)                            => model:map($node, $trackIds)
                        else
                            html:block($config, ., css:get-rendition(., ("tei-quote", "tei-quote2", "block-quote", css:map-rend-to-class(.))), .)                            => model:map($node, $trackIds)
                    case element(gap) return
                        if (desc) then
                            ...
                            if (@extent or @quantity) then
                                ...
                                return
                                                                html:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-gap", "tei-gap3", css:map-rend-to-class(.)), $content)                                => model:map($node, $trackIds)
                            else
                                html:inline($config, ., ("tei-gap", "tei-gap4", css:map-rend-to-class(.)), .)                                => model:map($node, $trackIds)
                    ...
                    case element(figDesc) return
                        html:inline($config, ., ("tei-figDesc", "tei-figDesc1", css:map-rend-to-class(.)), .)                        => model:map($node, $trackIds)
