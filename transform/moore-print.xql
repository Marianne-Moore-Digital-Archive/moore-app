...
module namespace model="http://www.tei-c.org/pm/models/moore/print";
            ...
            return
                    ...
                    case element(teiHeader) return
                        if ($parameters?mode eq 'metadata-doc') then
                            ...
                            if ($parameters?display='browse' or $parameters?mode='breadcrumb') then
                                html:block($config, ., ("tei-teiHeader", "tei-teiHeader4", css:map-rend-to-class(.)), .)
                            else
                                html:metadata($config, ., ("tei-teiHeader", "tei-teiHeader5", css:map-rend-to-class(.)), .)
                    case element(figure) return
                        if (head or @rendition='simple:display') then
                            html:block($config, ., ("tei-figure", "tei-figure1", css:map-rend-to-class(.)), .)
                        else
                    ...
                    case element(signed) return
                        if (parent::closer) then
                            html:block($config, ., ("tei-signed", "tei-signed1", "signed", css:map-rend-to-class(.)), .)
                        else
                        ...
                        if ($parameters?mode= ('toc', 'title')) then
                            ...
                            if (@facs) then
                                ...
                                if (starts-with(@facs, 'iiif:')) then
                                    ...
                                    html:break($config, ., css:get-rendition(., ("tei-pb", "tei-pb4", css:map-rend-to-class(.))), ., 'page', (concat(if(@n) then concat(@n,' ') else '',if(@facs) then                   concat('@',@facs) else '')))
                    ...
                    case element(anchor) return
                        if ($parameters?mode=('toc', 'breadcrumb')) then
                            ...
                            if (@corresp) then
                                ...
                                printcss:note($config, ., ("tei-anchor", "tei-anchor2", css:map-rend-to-class(.)), let $target := @corresp return root($parameters?root)/id($target)/node(), (), @n)
                            else
                                if (@type='note') then
                                    (: Process notes with @type=note (assuming the note is stand off, in the listAnnotation section in the same document) :)
                                    printcss:note($config, ., ("tei-anchor", "tei-anchor3", css:map-rend-to-class(.)), let $target := '#' || @xml:id return root($parameters?root)//listAnnotation/note[@target=$target]/node(), (), let $target := '#' || @xml:id return root($parameters?root)//listAnnotation/note[@target=$target]/@n/string())
                                else
                                    if (@xml:id) then
                    ...
                    case element(formula) return
                        if (@rendition='simple:display') then
                            html:block($config, ., ("tei-formula", "tei-formula1", css:map-rend-to-class(.)), .)
                        else
                    ...
                    case element(choice) return
                        if (sic and corr) then
                            printcss:alternate($config, ., ("tei-choice", "tei-choice1", css:map-rend-to-class(.)), ., corr[1], sic[1])
                        else
                            if (abbr and expan) then
                                printcss:alternate($config, ., ("tei-choice", "tei-choice2", css:map-rend-to-class(.)), ., expan[1], abbr[1])
                            else
                                if (orig and reg) then
                                    printcss:alternate($config, ., ("tei-choice", "tei-choice3", css:map-rend-to-class(.)), ., reg[1], orig[1])
                                else
                                    $config?apply($config, ./node())
                    ...
                    case element(note) return
                        if (@target) then
                            printcss:note($config, ., ("tei-note", "tei-note1", css:map-rend-to-class(.)), ., (), 'g')
                        else
                            if (not($parameters?mode eq 'metadata-doc')) then
                                printcss:note($config, ., ("tei-note", "tei-note2", css:map-rend-to-class(.)), ., @place, 'n')
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(dateline) return
                        html:block($config, ., ("tei-dateline", "tei-dateline1", css:map-rend-to-class(.)), .)
                    case element(back) return
                        html:block($config, ., ("tei-back", "tei-back1", css:map-rend-to-class(.)), .)
                    ...
                    case element(trailer) return
                        html:block($config, ., ("tei-trailer", "tei-trailer1", css:map-rend-to-class(.)), .)
                    case element(titlePart) return
                        html:block($config, ., css:get-rendition(., ("tei-titlePart", "tei-titlePart1", css:map-rend-to-class(.))), .)
                    ...
                    case element(app) return
                        if ($parameters?mode='toc') then
                            ...
                            printcss:alternate($config, ., ("tei-app", "tei-app2", css:map-rend-to-class(.)), ., lem, rdg)
                    ...
                    case element(roleDesc) return
                        html:block($config, ., ("tei-roleDesc", "tei-roleDesc1", css:map-rend-to-class(.)), .)
                    case element(opener) return
                        html:block($config, ., ("tei-opener", "tei-opener1", css:map-rend-to-class(.)), .)
                    case element(speaker) return
                        html:block($config, ., ("tei-speaker", "tei-speaker1", css:map-rend-to-class(.)), .)
                    case element(region) return
                        html:inline($config, ., ("tei-region", "tei-region1", css:map-rend-to-class(.)), .)
                    case element(imprimatur) return
                        html:block($config, ., ("tei-imprimatur", "tei-imprimatur1", css:map-rend-to-class(.)), .)
                    ...
                    case element(fileDesc) return
                        if ($parameters?display='browse') then
                            (
                                html:block($config, ., ("tei-fileDesc", "tei-fileDesc1", css:map-rend-to-class(.)), (titleStmt, editionStmt, publicationStmt)),
                                (: Output abstract containing demo description :)
                                html:block($config, ., ("tei-fileDesc", "tei-fileDesc2", "sample-description", css:map-rend-to-class(.)), ../profileDesc/abstract)
                            )

                        else
                            html:block($config, ., ("tei-fileDesc", "tei-fileDesc3", css:map-rend-to-class(.)), .)
                            ...
                            if ($parameters?mode='register') then
                                ...
                                (
                                    ...
                                    if ($parameters?mode='register-details') then
                                        ...
                                        return
                                                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-place", "tei-place6", css:map-rend-to-class(.)), $content)
                                    else
                                        ()
                                )
                    ...
                    case element(floatingText) return
                        html:block($config, ., ("tei-floatingText", "tei-floatingText1", css:map-rend-to-class(.)), .)
                    case element(sp) return
                        html:block($config, ., ("tei-sp", "tei-sp1", css:map-rend-to-class(.)), .)
                    case element(table) return
                        html:table($config, ., ("tei-table", "tei-table1", css:map-rend-to-class(.)), .)
                    case element(group) return
                        html:block($config, ., ("tei-group", "tei-group1", css:map-rend-to-class(.)), .)
                    case element(cb) return
                        html:break($config, ., ("tei-cb", "tei-cb1", css:map-rend-to-class(.)), ., 'column', @n)
                    ...
                    case element(orgName) return
                        html:inline($config, ., ("tei-orgName", "tei-orgName2", css:map-rend-to-class(.)), .)
                    case element(listBibl) return
                        if (bibl) then
                            html:list($config, ., ("tei-listBibl", "tei-listBibl1", css:map-rend-to-class(.)), ., ())
                        else
                            html:block($config, ., ("tei-listBibl", "tei-listBibl2", css:map-rend-to-class(.)), .)
                    ...
                    case element(author) return
                        if (ancestor::teiHeader) then
                            html:block($config, ., ("tei-author", "tei-author1", css:map-rend-to-class(.)), .)
                        else
                    ...
                    case element(l) return
                        if (not(node())) then
                            ...
                            html:break($config, ., ("tei-l", "tei-l1", css:map-rend-to-class(.)), ., 'line', ())
                        else
                            if (@rend) then
                                let $params := 
                                    map {
                                ...
                                return
                                                                html:block(map:merge(($config, map:entry("template", true()))), ., css:get-rendition(., ("tei-l", "tei-l2", "line", css:map-rend-to-class(.))), $content)
                            else
                                let $params := 
                                    map {
                                        "content": for $x in ./node() return if (contains(path($x), 'text()')) then ext-html:replaces-spaces($x) else if (contains(path($x), 'comment()')) then () else $x
                                ...
                                return
                                                                html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-l", "tei-l3", "line", css:map-rend-to-class(.)), $content)
                    case element(closer) return
                        html:block($config, ., ("tei-closer", "tei-closer1", css:map-rend-to-class(.)), .)
                    case element(p) return
                        if (parent::msDesc and not(preceding-sibling::p)) then
                            ...
                            if (ancestor::note) then
                                html:inline($config, ., ("tei-p", "tei-p2", css:map-rend-to-class(.)), .)
                            else
                    ...
                    case element(q) return
                        if (l) then
                            html:block($config, ., css:get-rendition(., ("tei-q", "tei-q1", "block-quote", css:map-rend-to-class(.))), .)
                        else
                            if (ancestor::p or ancestor::cell or ancestor::cit) then
                                ...
                                html:block($config, ., css:get-rendition(., ("tei-q", "tei-q3", "block-quote", css:map-rend-to-class(.))), .)
                    case element(epigraph) return
                        html:block($config, ., ("tei-epigraph", "tei-epigraph1", css:map-rend-to-class(.)), .)
                    case element(lb) return
                        html:break($config, ., css:get-rendition(., ("tei-lb", "tei-lb1", css:map-rend-to-class(.))), ., 'line', @n)
                    case element(docTitle) return
                        html:block($config, ., css:get-rendition(., ("tei-docTitle", "tei-docTitle1", css:map-rend-to-class(.))), .)
                            ...
                            if (parent::listPerson and $parameters?mode='register') then
                                ...
                                if (parent::listPerson) then
                                    (
                                        ...
                                        (: Variant names :)
                                        html:block($config, ., ("tei-person", "tei-person4", css:map-rend-to-class(.)), persName[@type='variant']),
                                        (: Images :)
                                        html:block($config, ., ("tei-person", "tei-person5", css:map-rend-to-class(.)), figure),
                                        (: Additional notes :)
                                        ...
                                        if ($parameters?mode='register-details') then
                                            ...
                                            return
                                                                                        html:block(map:merge(($config, map:entry("template", true()))), ., ("tei-person", "tei-person8", css:map-rend-to-class(.)), $content)
                                        else
                                            ()
                                    )
                    ...
                    case element(titlePage) return
                        html:block($config, ., css:get-rendition(., ("tei-titlePage", "tei-titlePage1", css:map-rend-to-class(.))), .)
                    case element(stage) return
                        html:block($config, ., ("tei-stage", "tei-stage1", css:map-rend-to-class(.)), .)
                    case element(persName) return
                        html:inline($config, ., ("tei-persName", "tei-persName2", css:map-rend-to-class(.)), .)
                    case element(lg) return
                        if (@type eq 'sideways') then
                            html:block($config, ., ("tei-lg", "tei-lg1", css:map-rend-to-class(.)), .)
                        else
                            if (@type eq 'vertical') then
                                html:block($config, ., ("tei-lg", "tei-lg2", css:map-rend-to-class(.)), .)
                            else
                                html:block($config, ., ("tei-lg", "tei-lg3", css:map-rend-to-class(.)), .)
                    case element(front) return
                        html:block($config, ., ("tei-front", "tei-front1", css:map-rend-to-class(.)), .)
                    case element(publicationStmt) return
                        if ($parameters?display='browse') then
                            html:block($config, ., ("tei-publicationStmt", "tei-publicationStmt1", css:map-rend-to-class(.)), .//availability)
                        else
                            html:block($config, ., ("tei-publicationStmt", "tei-publicationStmt2", css:map-rend-to-class(.)), .)
                    case element(placeName) return
                        html:inline($config, ., ("tei-placeName", "tei-placeName2", css:map-rend-to-class(.)), .)
                    case element(role) return
                        html:block($config, ., ("tei-role", "tei-role1", css:map-rend-to-class(.)), .)
                    ...
                    case element(postscript) return
                        html:block($config, ., ("tei-postscript", "tei-postscript1", css:map-rend-to-class(.)), .)
                    case element(edition) return
                        if (ancestor::teiHeader) then
                            html:block($config, ., ("tei-edition", "tei-edition1", css:map-rend-to-class(.)), .)
                        else
                            $config?apply($config, ./node())
                    ...
                    case element(head) return
                        if ($parameters?display='browse' or $parameters?mode='toc') then
                            ...
                            if (parent::figure) then
                                html:block($config, ., ("tei-head", "tei-head2", "figure-head", css:map-rend-to-class(.)), .)
                            else
                                if (parent::table) then
                                    html:block($config, ., ("tei-head", "tei-head3", "table-head", css:map-rend-to-class(.)), .)
                                else
                                    if (parent::lg) then
                                        html:block($config, ., ("tei-head", "tei-head4", "lg-head", css:map-rend-to-class(.)), .)
                                    else
                                        if (parent::list) then
                                            html:block($config, ., ("tei-head", "tei-head5", "list-head", css:map-rend-to-class(.)), .)
                                        else
                                            if (parent::div) then
                                                html:heading($config, ., ("tei-head", "tei-head6", css:map-rend-to-class(.)), ., count(ancestor::div))
                                            else
                                                html:block($config, ., ("tei-head", "tei-head7", css:map-rend-to-class(.)), .)
                    ...
                    case element(salute) return
                        if (parent::closer) then
                            ...
                            html:block($config, ., ("tei-salute", "tei-salute2", css:map-rend-to-class(.)), .)
                    ...
                    case element(date) return
                        if (($parameters?mode eq 'metadata-doc') and (ancestor::publicationStmt)) then
                            ...
                            if ($parameters?mode eq 'metadata-doc') then
                                ...
                                if (@when) then
                                    printcss:alternate($config, ., ("tei-date", "tei-date3", css:map-rend-to-class(.)), ., ., @when)
                                else
                                    html:inline($config, ., ("tei-date", "tei-date4", css:map-rend-to-class(.)), .)
                    case element(argument) return
                        html:block($config, ., ("tei-argument", "tei-argument1", css:map-rend-to-class(.)), .)
                        ...
                        if ($parameters?mode eq 'breadcrumb') then
                            ...
                            if ($parameters?display='browse') then
                                (
                                    ...
                                    html:block($config, ., ("tei-titleStmt", "tei-titleStmt5", css:map-rend-to-class(.)), subsequence(title, 2)),
                                    html:block($config, ., ("tei-titleStmt", "tei-titleStmt6", css:map-rend-to-class(.)), author)
                                )

                            else
                                html:block($config, ., ("tei-titleStmt", "tei-titleStmt7", css:map-rend-to-class(.)), .)
                    ...
                    case element(spGrp) return
                        html:block($config, ., ("tei-spGrp", "tei-spGrp1", css:map-rend-to-class(.)), .)
                    case element(body) return
                        (
                            html:index($config, ., ("tei-body", "tei-body1", css:map-rend-to-class(.)), 'toc', .),
                            html:block($config, ., ("tei-body", "tei-body2", css:map-rend-to-class(.)), .)
                        )
                    ...
                    case element(fw) return
                        if (ancestor::p or ancestor::ab) then
                            ...
                            html:block($config, ., ("tei-fw", "tei-fw2", css:map-rend-to-class(.)), .)
                    ...
                    case element(byline) return
                        html:block($config, ., ("tei-byline", "tei-byline1", css:map-rend-to-class(.)), .)
