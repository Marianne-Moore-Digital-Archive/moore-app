            ...
            return
                    ...
                    case element(date) return
                        if ($parameters?display='browse' or $parameters?mode='breadcrumb') then
                            fo:inline($config, ., ("tei-date", "tei-date1", css:map-rend-to-class(.)), .)
                        else
                            if (@when) then
                                fo:alternate($config, ., ("tei-date", "tei-date2", css:map-rend-to-class(.)), ., if (normalize-space(.)) then . else @when/string(), try { format-date(xs:date(@when), '[D1o] [MNn] [Y]', (session:get-attribute('lang'), 'en')[1], (), ()) } catch * { @when/string() })
                            else
                                fo:inline($config, ., ("tei-date", "tei-date3", css:map-rend-to-class(.)), .)
                    ...
                    case element(del) return
                        fo:inline($config, ., ("tei-del", "tei-del1", css:map-rend-to-class(.)), .)
                    case element(div) return
                        if ($parameters?mode = 'register') then
                            ...
                            return
                                                        fo:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-div", "tei-div1", css:map-rend-to-class(.)), $content)
                        else
                            if (parent::body or parent::front or parent::back) then
                                fo:section($config, ., ("tei-div", "tei-div2", css:map-rend-to-class(.)), .)
                            else
                    ...
                    case element(foreign) return
                        fo:inline($config, ., ("tei-foreign", "tei-foreign1", css:map-rend-to-class(.)), .)
                    ...
                    case element(gap) return
                        if (desc) then
                            ...
                            if (@extent or @quantity) then
                                ...
                                return
                                                                fo:inline(map:merge(($config, map:entry("template", true()))), ., ("tei-gap", "tei-gap3", css:map-rend-to-class(.)), $content)
                            else
                                fo:inline($config, ., ("tei-gap", "tei-gap4", css:map-rend-to-class(.)), .)
                    ...
                    case element(hi) return
                        fo:inline($config, ., css:get-rendition(., ("tei-hi", "tei-hi1", css:map-rend-to-class(.))), .)
                    ...
                    case element(l) return
                        fo:block($config, ., css:get-rendition(., ("tei-l", "tei-l1", css:map-rend-to-class(.))), .)
                    ...
                    case element(bibl) return
                        if ($parameters?mode='register-details') then
                            (
                                fo:block($config, ., ("tei-bibl", "tei-bibl1", css:map-rend-to-class(.)), .),
                                if ($parameters?mode='register-details') then
                                    ...
                                    return
                                                                        fo:block(map:merge(($config, map:entry("template", true()))), ., ("tei-bibl", "tei-bibl2", css:map-rend-to-class(.)), $content)
                                else
                                    ()
                            )
                            ...
                            if (parent::listBibl and $parameters?mode='register-overview') then
                                ...
                                return
                                                                fo:pass-through(map:merge(($config, map:entry("template", true()))), ., ("tei-bibl", "tei-bibl3", css:map-rend-to-class(.)), $content)
                            else
                                $config?apply($config, ./node())
                    ...
                    case element(note) return
                        if (parent::person or parent::place) then
                            fo:inline($config, ., ("tei-note", "tei-note1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?mode='toc' or ancestor::listAnnotation) then
                                fo:omit($config, ., ("tei-note", "tei-note2", css:map-rend-to-class(.)), .)
                            else
                                fo:note($config, ., ("tei-note", "tei-note3", css:map-rend-to-class(.)), ., @place, @n)
                    case element(opener) return
                        fo:block($config, ., ("tei-opener", "tei-opener1", css:map-rend-to-class(.)), .)
                    case element(p) return
                        fo:paragraph($config, ., css:get-rendition(., ("tei-p", "tei-p2", css:map-rend-to-class(.))), .)
                    case element(pb) return
                        if ($parameters?iiif-viewer='pb-facsimile') then
                            ...
                            if (@facs) then
                                ...
                                fo:break($config, ., css:get-rendition(., ("tei-pb", "tei-pb4", "pb", css:map-rend-to-class(.))), ., 'page', (concat(if(@n) then concat(@n,' ') else '',if(@facs) then                   concat('@',@facs) else '')))
                    ...
                    case element(quote) return
                        if (ancestor::p or ancestor::cell or ancestor::cit) then
                            (: If inside a paragraph, table cell or citation then rendered inline :)
                            fo:inline($config, ., css:get-rendition(., ("tei-quote", "tei-quote1", "inline-quote", css:map-rend-to-class(.))), .)
                        else
                            fo:block($config, ., css:get-rendition(., ("tei-quote", "tei-quote2", "block-quote", css:map-rend-to-class(.))), .)
                    ...
                    case element(teiHeader) return
                        fo:omit($config, ., ("tei-teiHeader", "tei-teiHeader2", css:map-rend-to-class(.)), .)
                    ...
                    case element(title) return
                        if ($parameters?mode='breadcrumb') then
                            fo:inline($config, ., ("tei-title", "tei-title1", css:map-rend-to-class(.)), .)
                        else
                            if ($parameters?mode='register-details' and $parameters?static) then
                                fo:link($config, ., ("tei-title", "tei-title2", css:map-rend-to-class(.)), string(), $parameters?context-path || "/letters/" || util:document-name(.) || '/1/index.html', map {})
                            else
                                if ($parameters?mode='register-details') then
                                    (: create a link to the document, respecting address-by-id configuration :)
                                    fo:link($config, ., ("tei-title", "tei-title3", css:map-rend-to-class(.)), string(),                          if ($global:address-by-id) then                             '../' || root(.)/descendant-or-self::TEI/@xml:id                                                      else                                 '../' || substring-after(document-uri(root(.)), $global:data-root || '/')                          , map {})
                                else
                                    if ($parameters?display='browse') then
                                        fo:heading($config, ., ("tei-title", "tei-title4", css:map-rend-to-class(.)), ., 5)
                                    else
                                        if (@key or @ref) then
                                            fo:alternate($config, ., ("tei-title", "tei-title5", css:map-rend-to-class(.)), ., ., let $key:= head((@key, @ref))  return collection($global:register-root)/id($key))
                                        else
                    ...
                    case element(titleStmt) return
                        fo:heading($config, ., ("tei-titleStmt", "tei-titleStmt2", css:map-rend-to-class(.)), ., ())
                    case element(trailer) return
                        fo:block($config, ., ("tei-trailer", "tei-trailer1", css:map-rend-to-class(.)), .)
                    case element(unclear) return
                        fo:inline($config, ., ("tei-unclear", "tei-unclear1", css:map-rend-to-class(.)), .)
